...
def low_poly_effect(frame, use_gpu=False):
    ...
    # === FAST POINT SELECTION: Corners + Edge-based points ===
    gray = cv2.cvtColor(small, cv2.COLOR_BGR2GRAY)

    # Use both corners and edge points for better structure preservation and speed
    points = []
    ...
    if corners is not None:
        points.extend([(int(p[0][0]), int(p[0][1])) for p in corners])

    # 2. Edge-based points (Sobel, biased random sampling)
    EDGE_POINT_RATIO = 0.5  # Fraction of points to sample from edges (tune for speed/quality)
    EDGE_POINT_COUNT = int(POLY_MAX_CORNERS * EDGE_POINT_RATIO)

    # Compute Sobel edge map
    sobelx = cv2.Sobel(gray, cv2.CV_64F, 1, 0, ksize=3)
    sobely = cv2.Sobel(gray, cv2.CV_64F, 0, 1, ksize=3)
    edge_mag = cv2.convertScaleAbs(np.abs(sobelx) + np.abs(sobely))

    # Normalize and threshold for strong edges
    edge_thresh = np.percentile(edge_mag, 90)
    edge_mask = (edge_mag > edge_thresh).astype(np.uint8)

    # Get edge pixel coordinates
    edge_coords = np.column_stack(np.where(edge_mask > 0))
    if len(edge_coords) > 0:
        # Randomly sample edge points
        idx = np.random.choice(len(edge_coords), min(EDGE_POINT_COUNT, len(edge_coords)), replace=False)
        edge_points = [(int(y), int(x)) for x, y in edge_coords[idx]]
    ...
    # === FAST TRIANGULATION: Optionally use grid-based triangles for speed ===
    USE_FAST_GRID_TRIANGULATION = False  # Set to True for max speed, False for Delaunay

    tris = []
    if USE_FAST_GRID_TRIANGULATION:
        # Use only grid points for a regular mesh (very fast, not as "random" looking)
        grid_rows = min(GRID_ROWS, h_small)
        grid_cols = min(GRID_COLS, w_small)
        grid_pts = np.array(add_grid_points(w_small, h_small, grid_rows, grid_cols, 0))
        for r in range(grid_rows - 1):
            for c in range(grid_cols - 1):
                idx = r * grid_cols + c
                p0 = grid_pts[idx]
                p1 = grid_pts[idx + 1]
                p2 = grid_pts[idx + grid_cols]
                p3 = grid_pts[idx + grid_cols + 1]
                # Two triangles per grid square
                tris.append([float(p0[0]), float(p0[1]), float(p1[0]), float(p1[1]), float(p2[0]), float(p2[1])])
                tris.append([float(p1[0]), float(p1[1]), float(p3[0]), float(p3[1]), float(p2[0]), float(p2[1])])
    else:
        # Delaunay (scipy or OpenCV)
        try:
            from scipy.spatial import Delaunay
            pts_arr = np.array(points_in_rect)
            tri = Delaunay(pts_arr)
            for simplex in tri.simplices:
                t = []
                for idx in simplex:
                    t.extend([float(pts_arr[idx][0]), float(pts_arr[idx][1])])
                tris.append(t)
        except ImportError:
...
def main():
    ...
    try:
        ...
        # Try to select the built-in webcam (FaceTime HD) instead of Continuity Camera/iPhone
        # Try indices 0, 1, 2 and pick the first available camera that is not Continuity Camera
        cap = None
        for cam_index in [1, 2, 0]:
            temp_cap = cv2.VideoCapture(cam_index)
            if temp_cap.isOpened():
                # Try to get camera name if possible (AVFoundation backend)
                backend = temp_cap.getBackendName() if hasattr(temp_cap, "getBackendName") else ""
                # On macOS, AVFoundation supports get(cv2.CAP_PROP_DEVICE_DESCRIPTION)
                cam_name = ""
                try:
                    cam_name = temp_cap.get(cv2.CAP_PROP_DEVICE_DESCRIPTION)
                except Exception:
                    pass
                # If camera name contains "Continuity" or "iPhone", skip it
                if isinstance(cam_name, str) and ("Continuity" in cam_name or "iPhone" in cam_name):
                    temp_cap.release()
                    continue
                cap = temp_cap
                print(f"Using camera index {cam_index} ({cam_name})")
                break
            else:
                temp_cap.release()
        if cap is None or not cap.isOpened():
            print("Error: Cannot open built-in webcam (tried indices 1, 2, 0)")
            exit(1)
